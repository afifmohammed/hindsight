...
namespace MediatR.Sagas
{
    public class InOrderForSagasToHandleMessages
    {
        ...
        {
            scope.Run(x => x.Publish(new PaymentSubmitted { Order = 1 }), x => x.Publish(new StockReserved { Order = 1 }));
        ...
        {
            scope.Run(x => x.Publish(new PaymentSubmitted { Order = 1 }), x => x.Publish(new StockReserved { Order = 2 }));
            SagaData<PaymentSagaState> state;
            dictionary.TryGetValue(1, out state);
